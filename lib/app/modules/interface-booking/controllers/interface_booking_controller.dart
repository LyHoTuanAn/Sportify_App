  ...
  Future<void> addToCart() async {
    ...
    try {
      ...
      if (response['status'] == 'success' || response['status'] == 1) {
        // Create booking info to pass to the next screen
        final courtName = selectedTimeSlots.first.split('-')[0];
        final yardData = yardAvailabilities.firstWhereOrNull(
          (yard) => yard.boatTitle == courtName,
        );

        // Navigate to booking price screen with booking info
        Get.toNamed('/booking-price', arguments: {
          'bookingInfo': {
            'venueName': yardData?.boatTitle ?? 'Sân thể thao',
        ...
        // Don't clear selections until after navigation since we need the data
      } else {
        Get.snackbar(
          'Lỗi',
          response['message'] ?? 'Có lỗi xảy ra khi thêm vào giỏ hàng.',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: Colors.red[100],
          colorText: Colors.red[900],
        );
